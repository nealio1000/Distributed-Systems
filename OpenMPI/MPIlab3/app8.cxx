...
void coordinator(int world_size)
{
	...
	// determine the size of each participation by dividing 100000 by 
	// the world size. It is imperative that the world_size divides this 
	// evenly
	int partition_size = 100000 / world_size;
	std::cout << "coordinator rank 0 partition size" << std::endl;
	
	// send out a partition of data to each node
	for(unsigned int i = 1; i < world_size; i++)
	{
		MPI_Send(values + partition_size * i, partition_size, MPI_INT, i,
			0, MPI_COMM_WORLD);
		std::cout << "coordinator rank 0 sent parition to rank " << i <<
			std::endl;
	}
	...
	// call a reduce operation to get the total average and then divide 
	// that by the world size
	float total_average = 0;
	MPI_Reduce(&average, &total_average, 1, MPI_FLOAT, MPI_SUM, 0, 
		MPI_COMM_WORLD);
	std::cout << "total average is " << total_average / world_size <<
		std::endl;
}
...
void participant(int world_rank, int world_size)
{
	...
	// get the partition size from the root and allocate memory as 
	// necessary
	int partition_size = 0;
	MPI_Bcast(&partition_size, 1, MPI_INT, 0, MPI_COMM_WORLD);
	std::cout << "participant rank " << world_rank << 
		" recieved partition size of " << partition_size << std::endl;
		
	// allocate memory for our partition
	int *partition = new int[partition_size];
	
	// recieve the partition from the root
	MPI_Recv(partition, partition_size, MPI_INT, 0, 0, MPI_COMM_WORLD,
		MPI_STATUS_IGNORE);
	std::cout << "participant rank " << world_rank << 
		" recieved partition from root" << std::endl;
	...
	for(unsigned int i = 0; i < partition_size; i++)
		total += partition[i];
	float average = (float) total / partition_size;
	std::cout << "participant rank" << world_rank << " average is " <<
		average << std::endl;
...
int main(int argc, char **argv)
{
	...
	// determine the rank in the world
	int world_rank;
	...
	// if we have a rank of zero of zero then we are the coordinator. if
	// not we are a participant in the task
	if(world_rank == 0)
